...
<div class="plot-container">
   {#if plotType == "r"}
   <CIPlot {clicked} {reset} x={rx} f={rf} limX={[-1, 1]} cix={cirx} cif={cirf} ci={rci} ciStat={sampCor} xLabel="Expected r for sample"/>
   {:else}
   <CIPlot {clicked} {reset} x={zx} f={zf} limX={[-5, 5]} cix={cizx} cif={cizf} ci={zci} ciStat={sampZ} xLabel="Expected z' for sample"/>
   {/if}
</div>
