...
<Axes xLabel="IQ" limX={limX} limY="{limY}">
   ...
   <!-- statistics -->
   <Segments xStart="{statValues}" xEnd="{statValues}" yStart="{yBottom}" yEnd="{yTop}" lineColor="#d0d0d0" />
   <ScatterSeries xValues="{statValues}" yValues="{yBottom}" borderColor="transparent" faceColor="#d0d0d0" />
   <TextLabels xValues={statValues} yValues={yBottom} labels={statLabels} pos={1} ></TextLabels>

   <!-- left boundary for outliers -->
   <Segments xStart="{[Q1]}" xEnd="{[outLeft]}" yStart="{[yTop[0]]}" yEnd="{[yTop[0]]}" lineColor="red" />
   <TextLabels xValues="{[Q1, outLeft]}" yValues="{[yTop[0], yTop[0]]}" labels="{["●", "|"]}" pos={0} faceColor="red" />
   ...
   <!-- sample points and boxplot -->
   <ScatterSeries xValues={values} yValues={yMid} faceColor="white" borderColor="{sampleColor}" borderWidth="{1.5}" />
   <BoxAndWhiskers values={values} boxPosition={2} boxSize={1} horizontal={true} />
   <Segments xStart="{[m]}" xEnd="{[m]}" yStart="{[2.5]}" yEnd="{[1.5]}" lineColor="{sampleColor}" lineType={2} />
