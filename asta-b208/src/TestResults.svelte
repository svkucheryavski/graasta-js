<script>
   ...
   import { pt, dt, qt } from 'mdatools/distributions';
   import { max } from 'mdatools/stat';
   import { ttest } from 'mdatools/tests';

   import {Lines, TextLegend, Segments} from 'svelte-plots-basic/2d';
   ...
   // run t-test
   $: testRes = ttest(sample, popH0Mean, 0.05, tail)
   ...
   $: t = Vector.seq(-10, 10, 0.1);
   $: xr = t.apply(v => v * popSE + popMean);
   $: fr = dt(t, sampSize - 1);
...
<TTestPlot {mainColor} {clicked} {reset} {testRes} {H0LegendStr} {limX} {xLabel} >
   <Lines xValues={xr} yValues={fr} lineColor={colorsPop.sample + "a0"} />
